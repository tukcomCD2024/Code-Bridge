...
internal class PdfRendererCore(
    ...
    private fun writeBitmapToCache(pageNo: Int, bitmap: Bitmap, shouldCache: Boolean = true) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val savePath = File(File(context.cacheDir, CacheManager.CACHE_PATH), pageNo.toString())
                FileOutputStream(savePath).use { fos ->
                    bitmap.compress(Bitmap.CompressFormat.JPEG, 75, fos) // Compress as JPEG
                }
            } catch (e: Exception) {
    ...
    fun prefetchPages(currentPage: Int, width: Int, height: Int) {
        val dynamicPrefetchCount = CommonUtils.calculateDynamicPrefetchCount(context, pdfRenderer!!)
