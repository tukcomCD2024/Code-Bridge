...
class PdfViewerActivity : AppCompatActivity(), PdfRendererView.OnPdfSelectedListener {
    ...
    // 선택한 PDF를 표시하는 뷰
    private var selectedPdfView: PdfRendererView? = null

    // 선택한 PDF의 비트맵
    private var selectedPdfBitmap: Bitmap? = null

    // PDF를 선택하는 메서드
    private fun selectPdf(pdfView: PdfRendererView) {
        // 이전에 선택한 PDF의 경계선을 원래 상태로 돌립니다.
        selectedPdfView?.background = null

        // 새로 선택한 PDF의 경계선을 파란색으로 표시합니다.
        pdfView.background = BorderDrawable(Color.BLUE)

        // 선택한 PDF를 저장합니다.
        selectedPdfView = pdfView

        // 선택한 PDF의 비트맵을 저장합니다.
        selectedPdfBitmap = pdfView.getBitmapByPage(0) // 첫 페이지의 비트맵을 가져옵니다.
    }
    ...
    private fun sendBitmapToPaintActivity() {
        selectedPdfBitmap?.let { bitmap ->
            ...
            val resultIntent = Intent().apply {
                putExtra("bitmap_uri", Uri.fromFile(tempFile).toString())
            }
    ...
    override fun onPdfSelected(page: Int) {
        Log.e("Page Selected", "Page: $page")
        //findViewById<FloatingActionButton>(R.id.completeButton). = "선택 완료"
    }
